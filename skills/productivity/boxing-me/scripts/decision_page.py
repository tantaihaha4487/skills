...
<body><main class="shell"><section class="hero"><div class="eyebrow">Decision brief</div><h1 id="title"></h1><p class="context" id="context"></p><div class="progress"><div class="track"><div class="bar" id="bar"></div></div><span class="count" id="count"></span></div></section><div class="toolbar"><button class="btn" id="recommend">Apply recommendations</button><button class="btn ghost" id="clear">Clear answers</button></div><form id="form" novalidate></form><section class="panel review"><div class="eyebrow">Review</div><h2>Your direction</h2><div class="summary" id="summary"></div><label for="overall" class="subtle">Anything the agent should change, avoid, or know?</label><textarea id="overall" class="notes" rows="4" placeholder="Optional overall feedback"></textarea><div class="actions"><button class="btn primary" id="save" type="button">Save decisions</button><button class="btn" id="copy" type="button">Copy agent brief</button><button class="btn" id="download" type="button">Download JSON</button></div><div class="status" id="status" role="status" aria-live="polite"></div></section><footer>Answers stay in this browser until you save, copy, or download them.</footer></main>
<script>
const spec={safe_json}; const key=`boxing-me:${{spec.id}}`; const state={{answers:{{}},notes:{{}},overall:""}};
const esc=s=>String(s??"").replace(/[&<>"']/g,c=>({{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}}[c]));
function optionMarkup(q,o){{const type=q.type==="multi"?"checkbox":"radio";return `<label class="option" data-q="${{esc(q.id)}}" data-o="${{esc(o.id)}}"><span class="option-head"><input type="${{type}}" name="${{esc(q.id)}}" value="${{esc(o.id)}}"><span><span class="label">${{esc(o.label)}}</span>${{o.recommended?'<span class="pill">Recommended</span>':''}}</span></span><span class="details">${{esc(o.details)}}</span>${{o.recommended?`<span class="recommend"><strong>Why this:</strong> ${{esc(o.recommendation_reason)}}</span>`:''}}</label>`}}
function render(){{document.title=`${{spec.title}} · Boxing Me`;document.querySelector('#title').textContent=spec.title;document.querySelector('#context').textContent=spec.context;document.querySelector('#form').innerHTML=spec.questions.map((q,i)=>`<section class="panel question" data-question="${{esc(q.id)}}"><div class="qtop"><div><div class="qnum">DECISION ${{i+1}}</div><h2>${{esc(q.prompt)}}</h2><p class="why">${{esc(q.why)}}</p></div>${{q.required?'<span class="required">Required</span>':''}}</div>${{q.type==='text'?`<textarea class="textinput" data-text="${{esc(q.id)}}" rows="4" placeholder="Write your answer"></textarea>`:`<div class="options">${{q.options.map(o=>optionMarkup(q,o)).join('')}}</div>${{q.allow_other?`<div class="other"><label class="subtle" for="other-${{esc(q.id)}}">Other or a variation</label><input id="other-${{esc(q.id)}}" class="textinput" data-other="${{esc(q.id)}}" placeholder="Describe what you want instead"></div>`:''}}`}}<label class="subtle" for="note-${{esc(q.id)}}">Notes for this decision</label><textarea id="note-${{esc(q.id)}}" class="notes" data-note="${{esc(q.id)}}" rows="2" placeholder="Optional detail"></textarea><div class="error" data-error="${{esc(q.id)}}"></div></section>`).join('');bind();restore();update();}}
...
function validate(show=false){{let ok=true;spec.questions.forEach(q=>{{let msg='';const a=state.answers[q.id];if(q.required&&!answered(q))msg='Choose or write an answer.';if(q.type==='multi'){{const n=(a||[]).length+(state.answers[`${{q.id}}__other`]?.trim()?1:0),min=q.min??(q.required?1:0),max=q.max??q.options.length;if(n<min)msg=`Choose at least ${{min}}.`;if(n>max)msg=`Choose no more than ${{max}}.`;}}if(msg)ok=false;const el=document.querySelector(`[data-error="${{CSS.escape(q.id)}}"]`);if(el)el.textContent=show?msg:'';}});return ok;}}
...
function brief(){{const p=payload();return [`Decision brief: ${{p.spec_title}}`,...p.answers.map(a=>{{const value=a.text||a.selected.map(x=>x.label).join(', ')||a.other||'Unanswered';return `- ${{a.prompt}}: ${{value}}${{a.other&&value!==a.other?`; Other: ${{a.other}}`:''}}${{a.notes?`; Notes: ${{a.notes}}`:''}}`;}}),p.overall_notes?`- Overall notes: ${{p.overall_notes}}`:null].filter(Boolean).join('\n');}}
function update(){{document.querySelectorAll('.option').forEach(el=>el.classList.toggle('selected',el.querySelector('input').checked));const done=spec.questions.filter(answered).length;document.querySelector('#bar').style.width=`${{done/spec.questions.length*100}}%`;document.querySelector('#count').textContent=`${{done}} of ${{spec.questions.length}} answered`;document.querySelector('#summary').innerHTML=spec.questions.map(q=>{{const a=payload().answers.find(x=>x.question_id===q.id),v=a.text||a.selected.map(x=>x.label).join(', ')||a.other||'Not answered';return `<div class="summary-row"><strong>${{esc(q.prompt)}}</strong><span>${{esc(v)}}</span></div>`}}).join('');validate(false);}}
...
document.querySelector('#clear').addEventListener('click',()=>{{if(!confirm('Clear every answer and note on this page?'))return;state.answers={{}};state.notes={{}};state.overall='';localStorage.removeItem(key);restore();update();document.querySelector('#status').textContent='Answers cleared.';}});
document.querySelector('#copy').addEventListener('click',async()=>{{try{{await navigator.clipboard.writeText(brief());document.querySelector('#status').textContent='Agent brief copied.';}}catch{{document.querySelector('#status').textContent='Copy was blocked. Use Download JSON instead.';}}}});
document.querySelector('#download').addEventListener('click',()=>{{const blob=new Blob([JSON.stringify(payload(),null,2)],{{type:'application/json'}}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${{spec.id}}-response.json`;a.click();URL.revokeObjectURL(a.href);document.querySelector('#status').textContent='Response downloaded.';}});
document.querySelector('#save').addEventListener('click',async()=>{{if(!validate(true)){{document.querySelector('#status').textContent='Complete the highlighted required decisions.';document.querySelector('.error:not(:empty)')?.scrollIntoView({{behavior:'smooth',block:'center'}});return;}}const button=document.querySelector('#save');button.disabled=true;try{{const response=await fetch('/api/save',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(payload())}});if(!response.ok)throw new Error();const result=await response.json();document.querySelector('#status').textContent=`Saved. Tell the agent you are done. (${{result.filename}})`;}}catch{{document.querySelector('#status').textContent='No local save server found. Your JSON was downloaded instead.';document.querySelector('#download').click();}}finally{{button.disabled=false;}}}});
render();
</script></body></html>'''
    return page.replace(
