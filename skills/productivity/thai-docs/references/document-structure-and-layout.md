...
- **Official document:** follow the supplied form, field order, identifiers, signature areas, and institution's typography. For a `บันทึกข้อความ` (`แบบที่ ๒๙`) internal memo, also use [internal-government-memo-layout.md](internal-government-memo-layout.md) when no conflicting applicable form is supplied. That memo profile is not a rule for every official document. If no form is available for another official-document type, clarify the type and mandatory requirements before claiming compliance. A generic formal report layout is only a draft fallback.
